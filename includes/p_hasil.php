...
for ($ii = 0; $ii < count($kriteria); $ii++) {
	$tbobot = $tbobot + $kriteria[$ii][4];
}

for ($ii = 0; $ii < count($kriteria); $ii++) {
	$w = $kriteria[$ii][4] / $tbobot;
	$bobot[] = array(round($w, 3));
}

for ($ii = 0; $ii < count($kriteria); $ii++) {
	if ($kriteria[$ii][3] != "benefit") {
		$bobot[$ii][0] =	$bobot[$ii][0] * -1;
	}
}
...
for ($i = 0; $i < count($alternatif); $i++) {
	...
	for ($ii = 0; $ii < count($kriteria); $ii++) {
		...
		if ($kriteria[$ii][3] == 'benefit') {
			if ($matriks_x[$i + 1][$ii + 1] > 0) {
				$jml = $matriks_x[$i + 1][$ii + 1] / max($arr);
			} else {
				$jml = 0;
			}
		} else {
...
for ($i = 0; $i < count($alternatif); $i++) {
	...
	for ($j = 0; $j < count($bobot); $j++) {
		...
		$jml *= pow($matriks_1[$i + 1][$j + 1], $bobot[$j][0]);
	}
	$thasil = $thasil + $jml;
...
$perbaikan_bobot = '<th class="text-center" width="40">NO</th><th class="text-center">NAMA KRITERIA</th><th class="text-center" width="100">NILAI</th>';
$perbaikan_bobot = '<thead><tr>' . $perbaikan_bobot . '</tr></thead><tbody>';
for ($i = 0; $i < count($bobot); $i++) {
	...
	<td class="text-center">' . $no . '</td>
	<td>' . $kriteria[$i][2] . '</td>
	<td class="text-center">' . $bobot[$i][0] . '</td>
	
	</tr>
	';
}
$perbaikan_bobot .= '</tbody>';
...
<div class="row">
	<div class="col-lg-12">
		...
		<div class="panel panel-primary">
			<div class="panel-heading">
				<h3 class="panel-title">Nilai Perbaikan Bobot</h3>
			</div>
			<div style="overflow-x:auto;width:100%;">
				<table class="table table-striped table-hover table-bordered" style="table-layout: fixed; width: 100%;">
